...
final class BenchmarkInfoScreen extends Screen {
    ...
    private static Component data(String value) {
        return Component.literal(value).withStyle(ChatFormatting.AQUA);
    }

    private static Component data(Component value) {
        return value.copy().withStyle(ChatFormatting.AQUA);
    }

    private static Component numberWithUnit(double value, String unit, String format) {
        if (!Double.isFinite(value)) return data("-");
        var result = Component.literal(String.format(Locale.ROOT, format, value))
                .withStyle(ChatFormatting.AQUA);
        return result.append(Component.literal(unit).withStyle(ChatFormatting.GREEN));
    }
